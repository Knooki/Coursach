...
void student::save_to_file(student new_stud) {
	wfstream fout(file_student, ios_base::app);
...
void student::show_info_stud(wstring group, wstring sort_type) {
	...
	if (array.size() != 0) {
		...
		wcout << L"Информация о студентах." << endl;
		print(array, group);
	}
	...
	if (array.size() != 0) {
		...
		if (buffer < 3)
			...
			case 3:
				...
				break;
			case 4:
				wcout << L"Введите искомую группу." << endl;
				while (1) {
					buffer = 1;
					rewind(stdin);
					getline(wcin, string_buffer, L'\n');
...
void student::add_stud() {
	...
	while (flag == 0) {
		...
		while (1) {
			while (!(wcin >> buffer.code_of_student) || wcin.peek() != L'\n' || buffer.code_of_student < 0 || buffer.code_of_student > 999999) {
				wcin.clear();
				wcin.ignore(numeric_limits<streamsize>::max(), L'\n');
				if (buffer.code_of_student < 0 || buffer.code_of_student > 999999) error_message(L"Недопустимый код студента.");
				else
					error_message(L"Вы можете ввести только цифры.");
			}
			for (register int i = 0; i < array.size(); i++)
	...
	if (array.size() != 0) {
		...
		if (flag == 0) {
			while (1) {
				...
				wcout << L"Код студента" << endl;
				while (1) {
					while (!(wcin >> buf.code_of_student) || wcin.peek() != L'\n' || buf.code_of_student < 0 || buf.code_of_student > 999999) {
						wcin.clear();
						wcin.ignore(numeric_limits<streamsize>::max(), L'\n');
						if (buf.code_of_student < 0 || buf.code_of_student > 999999) error_message(L"Код студента не может быть отрицательным.");
						else
							error_message(L"Вы можете ввести только цифры.");
					}
					for (register int i = 0; i < array.size(); i++)
...
void student::print(vector<student> array, wstring group) {
	...
	for (int i = 0; i < array.size(); i++)
	{
		if (array.at(i).speciality.length() > max_size_speciality)
			max_size_speciality = array.at(i).speciality.size();
		if (array.at(i).full_name.length() > max_size_full_name)
			max_size_full_name = array.at(i).full_name.size();
	}
	int table_width = space_students + ((max_size_full_name > 19) ? max_size_full_name + 1 : 19) + (max_size_speciality > 14 ? max_size_speciality + 1 : 14);
	wcout << L"┌" << wstring(table_width, L'─') << L"┐" << endl;
	wcout << L"│";
	SetConsoleTextAttribute(handle, FOREGROUND_INTENSITY);
	wcout << L"Код студента " << setw(max_size_full_name > 19 ? max_size_full_name + 1 : 19) << left << L"Полное имя студента"
		<< L"Дата рождения "
		<< setw(max_size_speciality > 14 ? max_size_speciality + 1 : 14) << L"Специальность "
		<< L"Группа "
		<< L"Факультет "
		<< L"Курс";
